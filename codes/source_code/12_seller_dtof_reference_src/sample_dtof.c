...
static td_s32 sample_dtof_start_vo(ot_vpss_grp vpss_grp[], td_u32 grp_num, const ot_size *in_size, td_bool is_dtof_pip)
{
    td_u32 i;
    td_s32 ret;
...
static td_s32 sample_dtof_one_sensor(int sensor_num)
{
    ...
    sample_comm_vi_get_size_by_sns_type(sns_type, &in_size);
    ret = sample_dtof_start_vo(vpss_grp, grp_num, &in_size, TD_FALSE);
    if (ret != TD_SUCCESS) {
        goto start_vo_failed;
    }
...
/*
    ...
    sensor0 + dtof
*/
static td_s32 sample_dtof_dtof_pip_and_rgb(int sensor_num)
{
    td_s32 ret;
    ...
    sample_comm_vi_get_size_by_sns_type(sns_type, &in_size);
    ret = sample_dtof_start_vo(vpss_grp, grp_num, &in_size, TD_TRUE);
    if (ret != TD_SUCCESS) {
        goto start_vo_failed;
    }
    ...
    ot_vi_pipe dtof_vi_pipe[1] = {vi_pipe[1]};
    ret = dtof_pip_init(dtof_vi_pipe, 1);
    if (ret != TD_SUCCESS) {
        sample_print("dtof init failed\n");
        goto dtof_init_failed;
    }

    ot_vi_dev dtof_vi_dev[1] = {sensor_num};
    ret = vi_bayerdump_pip(dtof_vi_pipe, dtof_vi_dev, 1, vpss_grp[0], vpss_chn);
    if (ret != TD_SUCCESS) {
        sample_print("start vi_bayerdump failed ret: 0x%x !\n", ret);
        goto vi_dump_failed;
    }
...
/*
    ...
    sensor0 + dtof0 + dtof1
*/
static td_s32 sample_dtof_two_dtof_pip_and_rgb()
{
    td_s32 ret;
    ...
    ot_vi_dev dtof_vi_dev[2] = {vi_dev[1], vi_dev[2]};
    ot_vi_pipe dtof_vi_pipe[2] = {vi_pipe[1], vi_pipe[2]};

    ret = dtof_pip_init(dtof_vi_pipe, 2);
    if (ret != TD_SUCCESS) {
        sample_print("dtof init failed\n");
        goto dtof_init_failed;
    }

    ret = vi_bayerdump_pip(dtof_vi_pipe, dtof_vi_dev, 2, vpss_grp[0], vpss_chn);
    if (ret != TD_SUCCESS) {
        sample_print("start vi_bayerdump failed ret: 0x%x !\n", ret);
        goto vi_dump_failed;
    }
...
static td_void sample_dtof_usage(char *prg_name)
{
    ...
    printf("note:\n");
    printf("    index 1~6 require dst_ip parameter.\n");
    printf("index:\n");
    ...
    printf("    (7) sensor0 + dtof0 pip             4lane sensor0 + 1lane sensor2.\n");
    printf("    (8) sensor0 + dtof1 pip             4lane sensor0 + 1lane sensor3.\n");
    printf("    (9) one sensor + two dtof pip       4lane sensor0 + 1lane sensor2 + 1lane sensor3.\n");
    printf("dst_ip:\n");
...
static td_s32 sample_dtof_execute_case(td_u32 case_index, char *dst_ip)
{
    ...
    switch (case_index) {
        ...
        case 6: /* 6 sensor0 + dtof0 + dtof1*/
            ...
            break;
        case 7: /* 7 sensor0 + dtof0 pip*/
            ret = sample_dtof_dtof_pip_and_rgb(2);
            break;
        case 8: /* 7 sensor0 + dtof1 pip*/
            ret = sample_dtof_dtof_pip_and_rgb(3);
            break;
        case 9: /* 7 sensor0 + dtof0 + dtof1 pip*/
            ret = sample_dtof_two_dtof_pip_and_rgb();
            break;
        default:
            ret = TD_FAILURE;
            break;
...
#endif
{
    ...
    if (argc != 0x2 && argc != 0x3) { /* 3:arg num */
    ...
    if (index >= 1 && index <= 6) {
        if (argc != 3) {
            printf("Error: index %d requires dst_ip parameter!\n\n", index);
            sample_dtof_usage(argv[0]);
            return TD_FAILURE;
        }
        dst_ip = argv[2];
