...
#ifndef DTOF_DUMP_RAW_DEPTH
#define DTOF_DUMP_RAW_DEPTH 2
#endif

#define DTOF_DUMP_SOURCE_PIPE 0
#define DTOF_DUMP_SOURCE_FE_OUT 1
#define DTOF_DUMP_SOURCE_BAS 2

#ifndef DTOF_DUMP_SOURCE
#define DTOF_DUMP_SOURCE DTOF_DUMP_SOURCE_PIPE
#endif
...
static const char *dtof_dump_source_name(td_void)
{
#if DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_FE_OUT
    return "fe_out";
#elif DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_BAS
    return "bas";
#else
    return "pipe";
#endif
...
static td_s32 dtof_get_dump_attr(ot_vi_pipe vi_pipe, ot_vi_frame_dump_attr *dump_attr)
{
#if DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_FE_OUT
    return ss_mpi_vi_get_pipe_fe_out_frame_dump_attr(vi_pipe, dump_attr);
#elif DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_BAS
    return ss_mpi_vi_get_pipe_bas_frame_dump_attr(vi_pipe, dump_attr);
#else
    return ss_mpi_vi_get_pipe_frame_dump_attr(vi_pipe, dump_attr);
#endif
}

static td_s32 dtof_set_dump_attr(ot_vi_pipe vi_pipe, const ot_vi_frame_dump_attr *dump_attr)
{
#if DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_FE_OUT
    return ss_mpi_vi_set_pipe_fe_out_frame_dump_attr(vi_pipe, dump_attr);
#elif DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_BAS
    return ss_mpi_vi_set_pipe_bas_frame_dump_attr(vi_pipe, dump_attr);
#else
    return ss_mpi_vi_set_pipe_frame_dump_attr(vi_pipe, dump_attr);
#endif
}

static td_s32 dtof_get_frame(ot_vi_pipe vi_pipe, ot_video_frame_info *frame_info, td_s32 milli_sec)
{
#if DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_FE_OUT
    return ss_mpi_vi_get_pipe_fe_out_frame(vi_pipe, frame_info, milli_sec);
#elif DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_BAS
    return ss_mpi_vi_get_pipe_bas_frame(vi_pipe, frame_info, milli_sec);
#else
    return ss_mpi_vi_get_pipe_frame(vi_pipe, frame_info, milli_sec);
#endif
}

static td_s32 dtof_release_frame(ot_vi_pipe vi_pipe, const ot_video_frame_info *frame_info)
{
#if DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_FE_OUT
    return ss_mpi_vi_release_pipe_fe_out_frame(vi_pipe, frame_info);
#elif DTOF_DUMP_SOURCE == DTOF_DUMP_SOURCE_BAS
    return ss_mpi_vi_release_pipe_bas_frame(vi_pipe, frame_info);
#else
    return ss_mpi_vi_release_pipe_frame(vi_pipe, frame_info);
#endif
}

static td_u32 dtof_pixel_format_bit_width(ot_pixel_format pixel_format)
{
    switch (pixel_format) {
...
static td_s32 dtof_unpack_raw_row(const td_u8 *src, td_u32 width, td_u32 bit_width, td_u16 *dst)
{
    td_u32 out_cnt = 0;

    if (bit_width == 8) {
        for (td_u32 i = 0; i < width; i++) {
            dst[out_cnt++] = src[i];
        }
        return TD_SUCCESS;
    }
    ...
    if (bit_width == 10) {
        for (td_u32 i = 0; i < width / 4; i++) {
            const td_u8 *tmp = src + 5 * i;
            td_ulong val = tmp[0] + ((td_u32)tmp[1] << 8) +
                ((td_u32)tmp[2] << 16) + ((td_u32)tmp[3] << 24) +
                ((td_ulong)tmp[4] << 32);

            dst[out_cnt++] = val & 0x3ff;
            dst[out_cnt++] = (val >> 10) & 0x3ff;
            dst[out_cnt++] = (val >> 20) & 0x3ff;
            dst[out_cnt++] = (val >> 30) & 0x3ff;
        }
        return TD_SUCCESS;
    }
    ...
    if (bit_width == 12) {
        for (td_u32 i = 0; i < width / 2; i++) {
            ...
            dst[out_cnt++] = val & 0xfff;
            dst[out_cnt++] = (val >> 12) & 0xfff;
        }
        return TD_SUCCESS;
    }
    ...
    if (bit_width == 14) {
        for (td_u32 i = 0; i < width / 4; i++) {
            const td_u8 *tmp = src + 7 * i;
            td_ulong val = tmp[0] + ((td_u32)tmp[1] << 8) +
                ((td_u32)tmp[2] << 16) + ((td_u32)tmp[3] << 24) +
                ((td_ulong)tmp[4] << 32) + ((td_ulong)tmp[5] << 40) +
                ((td_ulong)tmp[6] << 48);

            dst[out_cnt++] = val & 0x3fff;
            dst[out_cnt++] = (val >> 14) & 0x3fff;
            dst[out_cnt++] = (val >> 28) & 0x3fff;
            dst[out_cnt++] = (val >> 42) & 0x3fff;
        }
        return TD_SUCCESS;
    }
    ...
    if (bit_width == 16) {
        for (td_u32 i = 0; i < width; i++) {
            dst[out_cnt++] = src[2 * i] + ((td_u16)src[2 * i + 1] << 8);
        }
        return TD_SUCCESS;
    }
    ...
    ot_vi_pipe_attr* backup_pipe_attr, int pipe_attr_len)
{
    ...
    td_u32 u32nbit = 10;
    td_u32 u32rawdepth = DTOF_DUMP_RAW_DEPTH;
    ...
    for (td_u32 i = 0; i < bind_pipe->pipe_num; i++) {
        ret = dtof_get_dump_attr(bind_pipe->pipe_id[i], &backup_dump_attr[i]);
        if (ret != TD_SUCCESS) {
        ...
        ret = dtof_set_dump_attr(bind_pipe->pipe_id[i], &dump_attr);
        if (ret != TD_SUCCESS) {
...
#else
        printf("[DTOF_DBG] dump_source=%s keep vi_pipe %d attr pixfmt=%d compress=%d\n",
            dtof_dump_source_name(),
            bind_pipe->pipe_id[i],
            backup_pipe_attr[i].pixel_format,
            backup_pipe_attr[i].compress_mode);
#endif
    }
    return ret;
}
...
static td_u16* deal_frame_data(ot_video_frame_info *frame_info)
{
...
#ifdef DTOF_FORCE_UNPACK_10BIT
    td_u32 bit_width = 10;
#else
    td_u32 bit_width = dtof_pixel_format_bit_width(video_frame->pixel_format);
#endif
    ...
    for (td_s32 u32h = 0; u32h < video_frame->height; u32h++) {
        ...
        if (u32h > 0) {
            if (dtof_unpack_raw_row(pu8data, video_frame->width, bit_width, &pu16data[s32outcnt]) != TD_SUCCESS) {
                free(pu16data);
                ss_mpi_sys_munmap(virt_addr, size);
                virt_addr = TD_NULL;
                return TD_NULL;
            }
            s32outcnt += video_frame->width;
        }
...
static td_void dtof_debug_print_frame(unsigned int frame_idx, const DtofHandle *handle, td_u16 *raw_data)
{
...
static td_void dtof_debug_print_frame_info(unsigned int frame_idx, const ot_video_frame_info *frame_info)
{
    ...
    if (virt_addr == TD_NULL) {
        printf("[DTOF_FRAME] frame=%u mmap_failed phys0=0x%llx size=%llu\n",
            frame_idx,
    ...
    printf("[DTOF_FRAME] frame=%u w=%u h=%u stride0=%u pixfmt=%d compress=%d "
...
static td_void *dump_process(td_void *p)
{
    ...
    while (g_exit_flag == 0) {
        if (dtof_get_frame(vi_pipe, &astFrame, MILLI_SEC) != TD_SUCCESS) {
            printf("Linear:get vi_pipe %d frame err!\n", vi_pipe);
            continue;
        }

        dtof_debug_print_frame_info(frame_idx + 1, &astFrame);
        handle->data = deal_frame_data(&astFrame);
        //handle->data = malloc(2560*31*2);
        dtof_release_frame(vi_pipe, &astFrame);
        if (handle->data == NULL) {
            return TD_NULL;
        }
        ...
        DtofProcess(handle);
        frame_idx++;
        dtof_debug_print_frame(frame_idx, handle, handle->data);
...
#ifdef DTOF_FORCE_500PS_CONFIG
    printf("[DTOF_DBG] force gs1860 500ps config\n");
    gs1860_500ps_config(vi_pipe);
#endif
