...
class BoardYoloUdpNode(Node):
    ...
    def _normalize_detection(self, idx: int, item: dict[str, Any], image_w: int, image_h: int) -> dict[str, Any]:
        ...
        if not polygon:
            raise ValueError("parking YOLO payload requires mask_polygon")
        center = item.get("center_px") or self._polygon_center(polygon)
        ...
        det = {
            ...
            "polygon_source": str(item.get("polygon_source", "mask")),
        }
    ...
    @staticmethod
    def _polygon_center(points: list[list[float]]) -> list[float]:
        if not points:
            return [0.0, 0.0]
        return [
    ...
    def _slot_geometry(det: dict[str, Any], image_w: int, image_h: int) -> dict[str, Any]:
        ...
        corners = BoardYoloUdpNode._minimum_area_quad(pts)
        if len(corners) != 4:
            return {}
        ...
        return {
            ...
            "source": "mask_polygon_min_area_quad",
            "image_size": [image_w, image_h],
    ...
    @staticmethod
    def _minimum_area_quad(points: list[list[float]]) -> list[list[float]]:
