...
RECORD_ROOT=${RECORD_ROOT:-/home/ebaina/parking_sensor_records/yolo_vision_only}
...
exec ros2 launch parking_bridge parking.launch.py \
  ...
  enable_parking_planner:=false \
  enable_parking_controller_dry_run:=false \
  enable_stm32:=false
" > "$LOG" 2>&1 &
pid=$!
echo "$pid" > "$PID_FILE"
