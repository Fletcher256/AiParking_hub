...
def score_actions(state: dict, library: dict, responses: dict) -> list[dict]:
    ...
    for action in library.get("actions", []):
        measured, origin = select_response_record(action.get("id"), state, responses)
        if measured:
            measured = dict(measured)
            measured["_origin"] = origin
        pred, confidence, origin = predicted_state(state, action, measured)
        cost, parts = cost_state(pred, action, state, library, confidence, origin)
        ranked.append({
