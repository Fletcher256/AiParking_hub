...
Safety:
- Read-only verbs (PING / VER / STAT), state-light verbs (ST_PK/ST_SB/ST_ER),
  and the safety verb STOP run freely.
...
def validate_command(cmd: str, max_abs_d: float, max_gear: int,
                     min_ste: int = 45, max_ste: int = 140) -> tuple[str, bool, str | None]:
    ...
    if verb in {"PING", "VER", "STAT", "STOP", "GDIAG", "GYROCAL", "ST_PK", "ST_SB", "ST_ER"}:
        if cmd.upper() != verb:
    ...
    if verb == "ARC":
        ...
        if abs(d) > max_abs_d:
            return (verb, True, f"|D|={abs(d)} exceeds max_abs_d={max_abs_d}")
        if not (int(min_ste) <= ste <= int(max_ste)):
            return (verb, True, f"STE={ste} out of safe servo range [{int(min_ste)},{int(max_ste)}]")
        if not (0 <= v <= max_gear):
            return (verb, True, f"V={v} out of range [0,{max_gear}]")
        return (verb, True, None)

    return (verb, False, f"unknown V2 verb '{verb}' (allowed: PING/VER/STAT/TEL/GET/ZERO_*/GDIAG/GYROCAL/ST_*/STOP/MOVE/ARC)")
...
def main() -> int:
    ...
    parser.add_argument("--min-ste", type=int, default=45)
    parser.add_argument("--max-ste", type=int, default=140)
    args = parser.parse_args()

    verb, is_motion, error = validate_command(
        args.cmd, args.max_abs_d, args.max_gear, args.min_ste, args.max_ste)
    if error:
