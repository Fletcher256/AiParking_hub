...
# v2 of the live-VI-chn0 -> DtofProcess feed.
# v1 proved chn0 delivers the DECOMPRESSED full histogram (raw_nonzero ~76530,
# raw_max 1023) and DtofProcess produced real steady-state depth (UDP 23/23 valid),
# but release_chn_frame returned 0xa0108007 and leaked one VB block per frame, so it
# stalled after ~23 frames (the VB pool size).
#
# v2 fix: set the dtof chn0 attr to depth>0 AND raw/compress matching the bypass
# pipe at config time (so the started chn has a real user queue and the released
# frame format matches the chn attr), and read frames with a clean get/release loop
# (no runtime disable/enable). Falls back to runtime depth set if config depth did
# not take.

timestamp="$(date +%Y%m%d_%H%M%S)"
BUILD="${BUILD:-/home/ebaina/official_dtof_vi_chn_dtof2_${timestamp}}"
...
BINARY_NAME="${BINARY_NAME:-sample_dtof_official_vi_chn_dtof2_dbg}"
export BUILD ZIP
...
# sample_dtof.c is left unpatched: setting a RAW12/LINE chn attr at config time is
# rejected (vi set chn attr failed 0xa0108007). The runtime prepare below sets only
# chn depth (which is accepted), keeping the default YUV420 chn attr.

# ---- patch dtof_dumpraw.c: chn read loop feeding DtofProcess + UDP ----
python3 - <<'PY'
from pathlib import Path
...
static td_s32 dtof_vi_chn_dtof_prepare(ot_vi_pipe vi_pipe)
{
    ...
    /* If config-time depth did not take, fall back to a runtime disable/set/enable. */
    if (chn_attr.depth == 0) {
    ...
    printf("[DTOF_CHN_DTOF] using config-time chn depth=%u (no runtime disable/enable)\n", chn_attr.depth);
    fflush(stdout);
    return TD_SUCCESS;
}
...
static td_void *dtof_vi_chn_dtof_process(td_void *p)
{
    ...
    unsigned int rel_ok = 0;
    unsigned int rel_err = 0;
    td_s32 last_rel = 0;
    ...
    while (g_exit_flag == 0) {
        ...
        if (ret != TD_SUCCESS) {
            printf("[DTOF_CHN_DTOF] get_chn_frame err pipe=%d ret=0x%x rel_ok=%u rel_err=%u last_rel=0x%x\n",
                vi_pipe, ret, rel_ok, rel_err, last_rel);
            fflush(stdout);
            continue;
        }
        ...
        if (frame_idx == 12) {
            printf("[DTOF_CHN_DTOF] release stats after 12 frames: rel_ok=%u rel_err=%u last_rel=0x%x\n",
                rel_ok, rel_err, last_rel);
            fflush(stdout);
        }
        free(handle->data);
        handle->data = NULL;
...
print("VI_CHN_DTOF2 dtof_dumpraw.c patch inserted")
PY
...
if "CFLAGS += $(EXTRA_CFLAGS)" not in text:
    ...
    print("VI_CHN_DTOF2 EXTRA_CFLAGS hook inserted")
else:
    print("VI_CHN_DTOF2 EXTRA_CFLAGS hook already present")
PY

cd "$BUILD/src/dtof"
env PATH="$TOOLCHAIN" make clean >/tmp/dtof_vi_chn_dtof2_clean.log 2>&1 || true
env PATH="$TOOLCHAIN" make OS_TYPE=linux \
  ...
  all 2>&1 | tee /tmp/dtof_vi_chn_dtof2_build.log >/dev/null
